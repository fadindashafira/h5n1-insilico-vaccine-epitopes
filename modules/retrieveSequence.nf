...
/*
 * Module for sequence retrieval from NCBI
 */

process retrieveSequence {
    tag "${protein_type}:${accession}"
    
    publishDir "${params.outdir}/sequences", mode: params.publish_dir_mode
    ...
    output:
    path "${protein_type}_${accession}.fasta", emit: fasta
    
    script:
    def api_key = params.ncbi_api_key ?: ""
    def retry_delay = params.retry_delay ?: 3
    def max_retries = params.max_retries ?: 5
